...
export class DropdownDocsComponent implements OnInit, OnDestroy {
    rightMenu: DocsMenuItem[] = [
        ...
        {
            title: 'Add Box Dropdown',
            items: [
                {
                    label: 'Basic',
                    scrollTo: '#addbox-basic',
                    scrollOffset: 15
                }
            ]
        },
        {
            ...
            items: [
                {
                    label: 'Sub Group',
                    scrollTo: '#ddContentSubGroup',
                    scrollOffset: 15
                },
                {
                    label: 'Dynamic Title Icon',
                    scrollTo: '#ddContentDynamicIcon',
                    scrollOffset: 15
                },
                {
                    label: 'Filter Icon',
                    scrollTo: '#ddContentFilterIcon',
                    scrollOffset: 15
                },
                {
                    label: 'With Image and Icons',
                    scrollTo: '#ddContentForApp',
                    scrollOffset: 15
                }
            ]
        },
        ...
        },
        {
            ...
            items: [
                {
                    label: 'Disabled Dropdown Box',
                    scrollTo: '#ddDisabledDropdownBox',
                    scrollOffset: 0
                },
                {
                    label: 'Error',
                    scrollTo: '#ddErrorDropdownBox',
                    scrollOffset: 0
                }
            ]
        },
        {
            title: 'By placeholder',
            items: [
                {
                    label: 'Drop-down Box on management / analitic page header',
                    scrollTo: '#ddPageHeaderDropDownBox',
                    scrollOffset: 0
                }
            ]
        },
    ...
    dropdownControl2 = new FormControl();
    formDropDowns: FormGroup;
    errorMessage = 'Mandatory field';
    model: any;
    selected: Array<any>;
    selected2: Array<any>;
    selectedLang: Array<any>;
    multiSelected1: Array<any>;
    multiSelected3: Array<any>;
    multiSelected4: Array<any>;
    multiSelected: Array<any>;
    addboxSelected: Array<any>;
    addboxOptions: Array<any>;
    ...
    get optionsNoScroll(): Array<any> {
        return [...this.optionsNoIcons.filter((item, idx) => idx < 8)];
    }

    get languagesGrouped(): any[] {
        ...
        const languageArray = Object.keys(configLanguages).map(key => {
            return {
                id: key,
                title: configLanguages[key].VALUE,
                displayText: configLanguages[key].VALUE,
                isTop: configLanguages[key].IS_TOP
            };
        });
        return [
            {
                id: 'topLanguages',
                displayText: 'Top Languages',
                isGroup: true,
                childOptions: languageArray.filter(item => item.isTop)
            },
            {
                id: 'restLanguages',
                displayText: 'All the rest',
                isGroup: true,
                childOptions: [
                    ...[
                        {
                            id: 'None',
                            title: 'No Language',
                            displayText: 'No Language'
                        }
                    ],
                    ...languageArray.filter(item => !item.isTop)
                ]
            }
        ];
    }

    groupedOptions: Array<any> = OPTIONS_GROUPED;

    groupedSubGroupOptions: Array<any> = OPTIONS_GROUPED_SUBGROUP;
    ...
    selectedApp: Array<any>;

    optionsHeaderDropDown: Array<any> = [];
    optionsHeaderDropDownSelected: any;

    optionsInlineDropDown: Array<any> = [];
    optionsInlineDropDownSelected: any;
    ...
    countriesOptions: any[] = OPTIONS_COUNTRIES;

    breakdownOptions: any[] = [
        {id: 'none', displayText: 'None'},
        {id: 'title', displayText: 'Title'},
        {id: 'campaign', displayText: 'Campaign'},
        {id: 'country', displayText: 'Country'},
        {id: 'os', displayText: 'OS'},
        {id: 'deviceType', displayText: 'Device type'}
    ];

    placeholderComponentData = {};
    existedTitles = EXISTED_TITLES;
    campaignOptionsDataMock = [
        {
            id: 12,
        ...
        {
            id: 13,
            name: 'Oppa test more campaign',
            status: 2,
            titleId: 407551
        }
    ];

    customContentSelected = new FormControl();
    ...
    ngOnInit() {
        ...
        this.selected = this.multiSelected = this.addboxSelected = this.optionsAutoCompleteSelected = [];
        this.addboxOptions = [...this.options];
        this.optionsNoIcons = this.options.map(obj => {
            const objNoIcon = Object.assign({}, obj);
            delete objNoIcon.icon;
        ...
        this.optionsNoIconsWithImages = this.optionsNoIcons.map(obj => {
            const objNoIcon = Object.assign({}, obj);
            objNoIcon.image =
                'https://media.licdn.com/mpr/mpr/shrink_200_200' +
                '/AAEAAQAAAAAAAAjMAAAAJDRjYzEzMjRhLTI3MmItNGQ1Ny1iN2YxLTNkNjI2N2I3MWMzZg.png';
            return objNoIcon;
        });

        this.optionsHeaderDropDown = [
            'Rewarded Video by Ad Source, Last 7 Days',
            'Rewarded Video by App, Last 7 Days',
            'Rewarded Video by Country, Hopeless2: Cave Escape (Android), Last 7 Days',
            'US RV Hopeless 2'
        ];
        this.optionsHeaderDropDownSelected = [this.optionsHeaderDropDown[0]];

        this.optionsInlineDropDown = ['Hours', 'Days', 'Weeks', 'Month'];
    ...
    formInit(): void {
        this.formDropDowns = this.formBuilder.group({
            requiredDropDown: [this.selected, [Validators.required]],
    ...
    onSearchCampaignChange(searchKey) {
        this.optionsAutoComplete = [];
        this.loading = true;
        return of(this.campaignOptionsDataMock)
            .pipe(delay(1000))
            .subscribe(
                data => {
                    this.optionsAutoComplete = data.map(item => {
                        return {
                            id: item.id,
                            name: item.name
                        };
    ...
    onCampaignSelected(selected) {
        const campaign = this.campaignOptionsDataMock.find(item => item.id === selected[0].id);
        this.placeholderComponentData = {
            id: campaign.id,
            name: campaign.name,
            deliveryStatus: campaign.status,
            title: this.existedTitles.find(item => item.id === campaign.titleId)
        };
    }
