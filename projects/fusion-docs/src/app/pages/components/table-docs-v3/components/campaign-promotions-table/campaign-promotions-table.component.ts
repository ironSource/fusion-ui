...
import {delay, finalize, first, take, takeUntil} from 'rxjs/operators';
...
export class CampaignPromotionsTableComponent implements OnInit, OnDestroy {
    ...
    promotionsTableOption$ = new BehaviorSubject<TableOptions>({
    ...
    onLoadNextPageOfData() {
        ...
        of(true)
            .pipe(take(1), takeUntil(this.onDestroy$), delay(1000))
            .subscribe(_ => {
                this.promotionsTableRows = [...this.promotionsTableRows, ...this.promotionsTableRows.slice(0, 5)];
