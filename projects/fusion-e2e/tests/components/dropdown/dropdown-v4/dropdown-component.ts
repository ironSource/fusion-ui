...
export class DropdownComponent extends BaseDropdownComponent {
    ...
    getSelectedLabel({testId}: {testId: string}) {
        return this.page.getByTestId(TestIdsService.getTestId(testId, DropdownTestIdModifiers.BUTTON_CONTENT)).textContent();
    }

    async searchForItem({testId, searchTerm}: SearchItem) {
        await this.openDropdownComponent({testId});
        await this.page.getByTestId(TestIdsService.getTestId(testId, InputTestIdModifiers.FIELD)).last().fill(searchTerm);
        return this.page
    ...
    async isErrorText({testId}: {testId: string}) {
        const errorTextLocator = await this.page.getByTestId(TestIdsService.getTestId(testId, FieldHelpTextTestIdModifiers.TEXT)).count();
        return errorTextLocator > 0;
    }

    async isDisabled({testId}: {testId: string}) {
        const ddTriggerSelector = await this.page
            .getByTestId(TestIdsService.getTestId(testId, DropdownTestIdModifiers.TRIGGER))
    ...
    async clearAllOptions({testId}: {testId: string}) {
        await this.page.getByTestId(TestIdsService.getTestId(testId, DropdownTestIdModifiers.ACTION_CLEAR_ALL)).click();
    }

    isSelectAllChecked({testId}: {testId: string}) {
        return this.page
            .getByTestId(TestIdsService.getTestId(testId, DropdownTestIdModifiers.SELECT_ALL))
    ...
    async removeChipSelection({testId}: {testId: string}) {
        await this.page.getByTestId(TestIdsService.getTestId(testId, DropdownTestIdModifiers.BUTTON_CLEAR)).click();
    }
